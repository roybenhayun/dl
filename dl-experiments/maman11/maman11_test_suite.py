import torch
from maman11_1a import expand_as_impl, get_broadcastable
from maman11_c import broadcast_tensors_impl
...
environment = {
    "expand_as_function": expand_as_impl,
    "broadcastable_together_function": get_broadcastable,
    "broadcast_tensors_function": broadcast_tensors_impl,
    "verbose": True
}
...
def print_fail_and_increment_test_case_counter(expected, actual, code):
    ...
    if environment["verbose"]:
        if code == code_bad_error_handling:
            ...
            print(f"expected: {expected.shape}", f"actual: {actual.shape}")
